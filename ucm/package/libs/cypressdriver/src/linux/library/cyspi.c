        ...
        )	
{
    ...
    if (rStatus == CY_SPI_CONFIG_LEN){
        ...
        spiConfig->frequency = le32toh(localSpiConfig.frequency);
        ...
        )	
{
    ...
    wValue = be16toh(scbIndex << CY_SCB_INDEX_POS);
    wIndex = 0;
    wLength = CY_SPI_CONFIG_LEN;
    ...
    localSpiConfig.frequency = htole32(spiConfig->frequency);
...
void do_attention()
{
	system("/bin/echo 0 > /sys/class/gpio/gpio19/value");
	usleep(20);
}

void reset_attention()
{
	system("/bin/echo 1 > /sys/class/gpio/gpio19/value");
	usleep(20);
}
/*
